...
def AutoFillContent():
    ...
    CleanWithSend(driver.find_element(by.XPATH, Definitions.LocationInput_XPATH), Definitions.Location)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 起降备降场地 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.EmergencyProcessInput_XPATH), Definitions.EmergencyProcess)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 应急处置程序 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.SpecialReqInput_XPATH), Definitions.SpecialReq)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 特殊飞行保障需求 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.AbilitiesInput_XPATH), Definitions.Abilities)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 通信、导航和被监视能力 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.SpeedInput_XPATH), Definitions.Speed)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 飞行速度和进出空域方法 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.ControlInput_XPATH), Definitions.ControlAbilities)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 指挥控制链路无线电频率以及占用带宽 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.RadarInput_XPATH), Definitions.Radar)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 二次雷达应答机或有关自动监视设备代码 更改')
    CleanWithSend(driver.find_element(by.XPATH, Definitions.OtherInfoInput_XPATH), Definitions.Other)
    print(f'[INFO] {str(datetime.datetime.now())} 完成 其他必要信息 更改')
